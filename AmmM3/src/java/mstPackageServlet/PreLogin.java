 ...
 * @author MARCO
 */
@WebServlet(name = "PreLogin", urlPatterns = {"/preLogin.html"})
public class PreLogin extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if(null == session.getAttribute("loggedIn")){
            if(null == session.getAttribute("loggedInCliente") || null == session.getAttribute("loggedInVenditore")){
                request.getRequestDispatcher("login.html").forward(request, response);
            }
        }
        else{
            if(null != session.getAttribute("loggedInCliente"))
                request.getRequestDispatcher("cliente.html").forward(request, response);
            else if(null != session.getAttribute("loggedInVenditore"))
